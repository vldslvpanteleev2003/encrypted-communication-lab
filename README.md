...
1. The client sends the string `"get_task"`.
2. The server responds with the task value read from `task.txt`.
3. Based on the received task:
...
4. The server receives the packet, extracts the IV, decrypts the payload, and logs the result into `receivedata.txt` along with a timestamp.
